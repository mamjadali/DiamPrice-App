//
//  DPDiamondsViewController_iPAD.m
//  DiamPrice
//
...
#import "DPDiamondsViewController_iPAD.h"
#import "DPCollectionViewCell.h"
#import "DPShapesViewController_iPAD.h"
#import "DPAboutViewController_iPAD.h"
#import "MJSharedClient.h"
#import "MJWebServiceClient.h"
#import "DPDiamondCellHeaderView.h"
#import <MessageUI/MFMailComposeViewController.h>
#import "DPSettingsViewController_iPAD.h"
#define ROOT_FOLDER [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0]

@interface DPDiamondsViewController_iPAD ()<UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,MFMailComposeViewControllerDelegate>
...
@implementation DPDiamondsViewController_iPAD
...
{
    [super viewDidLoad];
    UIColor *blue = [UIColor colorWithRed:76.0/255.0 green:190.0/255.0 blue:241.0/255.0 alpha:1];
...
- (IBAction)shapesButtonPressed:(id)sender {
	DPShapesViewController_iPAD *aboutShape=[DPShapesViewController_iPAD new];//initWithTITLE:@"" HTML:html];
//	[aboutShape setContentSizeForViewInPopover:CGSizeMake(320, 522)];
	if (popOverShapes==nil)
	{
		popOverShapes = [[UIPopoverController alloc] initWithContentViewController:aboutShape];
		popOverShapes.delegate = self;
	}
	popOverIsActive = YES;
    
	[popOverShapes presentPopoverFromRect:shapesButton.frame inView:self.calTableView permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
}
...
- (IBAction)addToCollectionButtonPressed:(id)sender {
    [super addToCollectionButtonPressed:nil];
...
//    return [[MJSharedClient sharedInstance].myCollection count];
}
...
- (IBAction)emailButtonPressed:(id)sender{
    ...
    if ([self.diamondArray count]>0) {
        [actionSheet showFromRect:mailButton.frame inView:self.calTableView animated:YES];
    }
...
- (IBAction)infoButtonPressed:(id)sender {
    DPAboutViewController_iPAD *aboutView=[DPAboutViewController_iPAD new];//initWithTITLE:@"" HTML:html];
//	[infoView setContentSizeForViewInPopover:CGSizeMake(320, 522)];
	if (popOverAbout==nil)
	{
		popOverAbout = [[UIPopoverController alloc] initWithContentViewController:aboutView];
		popOverAbout.delegate = self;
	}
	popOverIsActive = YES;
    
	[popOverAbout presentPopoverFromRect:aboutButton.frame inView:self.calTableView permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
...
- (IBAction)cameraButtonPressed:(id)sender {
    if(self.isselectedRow)
	{
        ...
        [choicesSheet showFromRect:cameraButton.frame inView:self.calTableView animated:YES];
...
-(IBAction)settingsButtonPressed:(id)sender {
    ...
    DPSettingsViewController_iPAD *settingView=[DPSettingsViewController_iPAD new];
...
//    [initViewController setContentSizeForViewInPopover:CGSizeMake(600, 522)];
    initViewController.navigationBar.barTintColor = [UIColor blackColor];
    initViewController.preferredContentSize = CGSizeMake(423, 275);

	if (popOverSetting==nil)
	{
		popOverSetting = [[UIPopoverController alloc] initWithContentViewController:initViewController];
		popOverSetting.delegate = self;
    
	}
	popOverIsActive = YES;
    
	[popOverSetting presentPopoverFromRect:settingButton.frame inView:self.calTableView permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
...
//    NSLog(@"orin is :%ld",orientation);
    
    if (orientation>2) {
        NSLog(@"for Landscape");
        
        self.pickerView.frame = CGRectMake(640, 43, 384, 150);
        self.addToCollectView.frame = CGRectMake(641, 692, 382, 71);
        self.calculatorView.frame = CGRectMake(640, 200, 384, 492);
        self.logoView.frame = CGRectMake(1, 43, 637, 235);
        self.calTableView.frame = CGRectMake(-1, 278, 638, 492);

    }else{
        self.pickerView.frame = CGRectMake(0, 304, 382, 230);
        self.addToCollectView.frame = CGRectMake(0, 513, 382, 71);
        self.calculatorView.frame = CGRectMake(383, 43, 384, 541);
        self.logoView.frame = CGRectMake(1, 43, 381, 259);
        self.calTableView.frame = CGRectMake(-1, 587, 769, 437);
